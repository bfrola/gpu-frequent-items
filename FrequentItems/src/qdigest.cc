...
QD_node * QD_Buffer(QD_type * qd, unsigned int item, QDWeight_t wt){
...
void QD_InsertR(QD_admin * qda, unsigned int item, QDWeight_t weight) {
...
void QD_MergeBuf(QD_type * fqd, QD_type * qd) {
	...
	while (qd->a->bufhead) {
		pt=QD_UnBuffer(qd);
		QD_Insert(fqd,(unsigned int) pt->kids[0],pt->wt);
...
void QD_Insert(QD_type * qd, unsigned int item, QDWeight_t wt) {
...
void QD_Show(QD_type *qd, unsigned int id, QD_node * point, int depth) {
	...
	for (i=0; i<depth; i++)
		printf(" ");
	printf("%d [%d--%d] ct = %f \n",(int) point,id,depth, (double) point->count);
	for (i=0;i<=1; i++)
		if (point->kids[i])
...
QDWeight_t QD_LBoundRank(QD_type * qd, unsigned int item)
{
...
void QD_InsertDecayedR(QD_admin * qda, unsigned int item, double decwt) {
	unsigned int mask, b;
...
void QD_InsertDecayed(QD_type * qd, unsigned int item, double itime) {
...
void QDSW_Insert(QDSW_type * sw, unsigned int item, unsigned int time){
	int i,j;
