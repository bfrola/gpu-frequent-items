...
// Disable occourring warning messages
...
int main(int argc, char **argv) 
{
	...
	// size_t stNumberOfPackets = 10 * 1000 * 1000; // 10M
	size_t stNumberOfPackets = 100 * 1000;
	size_t stRuns = 1;
	double dPhi = 0.001;
	uint32_t u32Depth = 4;
	...
	std::string sPcapFile = "login.pcap";
#else
	double dSkew = 1.0;
#endif
	...
	Stats SFreq, SQD, SCGT, SLC, SLCD, SLCL, SLCU, SCMH, SCCFC, SGK4, GPUFreqStats;
	...
	//GPUFrequent* gpuFreq = new GPUFrequent();
	//gpuFreq->init( dPhi );
	...
	PrintEndCheckPoint( );
	std::cout << "UPDATE start [" << stRuns * stRunSize << "] elements" << std::endl;

	for (size_t run = 1; run <= stRuns; ++run)
	{
		PrintStartCheckPoint( "Exact" );
		...
		//PrintCheckPoint( "GPUFrequent..." );
		//StartTheClock(nsecs);
		//for (size_t i = stStreamPos; i < stStreamPos + stRunSize; ++i)
		//{
		//	gpuFreq->update( data[i] );
		//}
		//GPUFreqStats.dU += StopTheClock(nsecs);
		...
		std::cerr << std::endl << "Run: " << run << ", Exact: " << hh << std::endl;

		PrintStartCheckPoint( "Output processing..." );
