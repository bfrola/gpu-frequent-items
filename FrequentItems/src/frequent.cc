...
void Freq_Update(freq_type * freq, int newitem) 
{
  ...
  // Controllo sul valore se > 0
  if (newitem>0) diff=1;
  else 
    { // Elemento negativo - nessun elemento negativo con dati casuali
      (newitem=-newitem);
      diff=-1;
    }

  // Hash table: ricerca di un bucket per il dato

  i=hash31(freq->a,freq->b,newitem) % freq->tblsz; // Calcola l'hash
  il=freq->hashtable[i]; // Hast table basata su liste linkate


  while (il!=NULL) { // Cerca un elemento vuoto della lista
    if ((il->item)==newitem) 
      break;
    il=il->nexti;
  }
  ...
  if (il==NULL) // NON è in T
	...
	if (diff==1)
	{
		if (il->parentg->diff==0)
			RecycleCounter(freq,il); // Primo elemento 
		else
			FIncrementCounter(il); // Incrementa il contatore (COND 1)
