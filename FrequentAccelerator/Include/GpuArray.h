...
// Modified by Bernardino Frola, IsisLab, University of Salerno
// 13/05/2011: Added clause USE_VBO. Allow to use this class without linking OpenGL libs
// 16/05/2011: Added m_UseExternalHostArray
...
public:
    ...
    void alloc(size_t size, bool vbo=false, bool doubleBuffer=false, 
		bool useElementArray=false, T* hostData = NULL);
	// TODO 9: Add/modify alloc without host allocation
    void free();
    ...
    __host__ __device__ size_t getSize() const { return m_size; }
...
template <class T> 
void
GpuArray<T>::alloc(size_t size, bool vbo, bool doubleBuffer, bool useElementArray, T* hostData)
{
    m_size = size;
	...
	{
		m_hptr = hostData;
		m_UseExternalHostArray = true;
	}
	else
	{
		allocHost();
		m_UseExternalHostArray = false;
	}
...
GpuArray<T>::allocDevice()
{
	...
	//std::cout << "Allocated: " << m_size*sizeof(T) << " bytes" << std::endl; // TODO: delete this
}
